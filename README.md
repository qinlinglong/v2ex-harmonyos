...
- 最近/最热/XNA/节点/我的 Tab、列表刷新、横向切换和节点分页；
- 主题和用户搜索、主题详情、Markdown 与 V2EX 原生格式、图片、回复、回复编辑、追加内容、默认/最新/热度回复排序；
- 关联回复筛选、主题忽略/举报、用户屏蔽、通知删除、Bilibili 与 Imgur 图片上传（系统相册选择后自动插入 Markdown）和可配置 V2EX 域名；
...
- 应用桌面图标和启动图使用 V 标识矢量资源，Bundle 为 `io.github.qinlinglong.v2ex`。
...
- 回复框提供常用表情选择，插入的图片链接与网页端原生格式兼容；评论显示真实楼层编号并支持楼层引用。
- 系统状态栏采用浅色原生系统栏样式，搜索输入框提供明确的占位与正文文字颜色。
- 首页板块支持本地持久化的显示/隐藏和上下移动，可启用技术、创意、好玩、Apple、酷工作、交易、城市、问与答、全部等节点板块；节点板块使用 V2EX 节点主题接口加载。
...
## 致谢
感谢 [V2EX_Polish](https://github.com/coolpace/V2EX_Polish) 开源项目，在表情处理、评论楼层和回复交互优化等方面提供了有价值的参考。
